...
import (
	...
	"github.com/fren-chain/go-frenchain/console"
...
func remoteConsole(ctx *cli.Context) error {
	...
	if endpoint == "" {
		...
		if path != "" {
			...
			} else if ctx.GlobalBool(utils.FrenTestFlag.Name) {
				path = filepath.Join(path, "frentest")
			} else if ctx.GlobalBool(utils.FrenChainFlag.Name) {
				path = filepath.Join(path, "frenchain")
